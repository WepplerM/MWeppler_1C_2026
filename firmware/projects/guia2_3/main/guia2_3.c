...
void EnviarDistanciaUART(uint16_t distancia){
    ...
    char buffer[10];  // suficiente para "123 cm\r\n"
    
    uint8_t* str = UartItoa(distancia, 10);

    // Formato: "123 cm\r\n"
    sprintf(buffer, "%03s cm\r\n", str);

    UartSendString(UART_PC, buffer);
}

void comunicacionUART(void *medicion){
...
void app_main(void){
	...
	xTaskCreate(Task_UART, "UART", BAUD_RATE, NULL, 2, NULL);
